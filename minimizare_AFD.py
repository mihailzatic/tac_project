...
class AFDMinimizerApp:
    ...
    def show_minimized_afd(self, afd):
        ...
        # Sort the alphabet to ensure consistent display order
        columns = sorted(afd.alphabet)
        ...
        for i, state in enumerate(afd.states):
            ...
            for j, symbol in enumerate(columns):  # Use the sorted alphabet
                transition_value = afd.transitions[state].get(symbol, "")  # Fetch the correct transition
